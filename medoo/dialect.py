...
class Dialect:
	...
	def eq(cls, field, value): # pylint: disable=invalid-name
		...
		if isinstance(value, (tuple, list)):
			return '{} IN ({})'.format(field, ','.join([cls.value(v) for v in value]))
	...
	def like(cls, field, value, addperct = True):
		...
		if isinstance(value, (tuple, list)):
			...
			return '({})'.format(' OR '.join(['{} LIKE {}'.format(field, cls.value(v)) for v in value]))

		if addperct and isinstance(value, six.string_types) \
	...
	def ne(cls, field, value): # pylint:disable=invalid-name
		...
		if isinstance(value, (tuple, list)):
			return '{} NOT IN ({})'.format(field, ','.join([cls.value(v) for v in value]))
	...
	def between(cls, field, value):
		...
		return '{} BETWEEN {} AND {}'.format(field, cls.value(value[0]), cls.value(value[1]))
