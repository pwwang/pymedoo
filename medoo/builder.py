...
class Field(Term):
    ...
    def __init__(self, field='*', table=None, schema=None):
        ...
        if len(parts) == 1:
            ...
            if table:
                raise FieldParseError('Confusing table specified in '
    ...
    def parse(fieldstr, context=None):
        ...
        if context == 'select':
            ...
            if not matching:
                raise FieldParseError('Unrecognized field string: '
                                      '{}'.format(fieldstr))
    ...
    def __str__(self):
        ...
        if self.func:
            if hasattr(Builder.DIALECT, self.func.lower()):
                ...
                ret = '{}({}{})'.format(self.func.upper(), 'DISTINCT '
                                        if self.distinct else '', ret)
        if self.alias:
...
class Order(Term):
    ...
    def __str__(self):
        return ','.join([str(OrderTerm(key, val))
                         for key, val in self.orders.items()])

class OrderTerm(Term):
    """Terms in ORDER"""
    REGEX_KEY = (r'^\s*((?:[\w_]+\.)?(?:[\w_]+\.)?(?:[\w_]+|\*))'
                 r'\s*(?:\|([\w_.]+))?\s*$')
    def __init__(self, key, val):
...
class Set(Term):
    ...
    def __str__(self):
        return ','.join([str(SetTerm(key, val))
...
class Join(Term):
    ...
    def __str__(self):
        return ' '.join([str(JoinTerm(key, val, self.maintable))
...
class JoinTerm(Term):
    ...
    def __init__(self, key, val, maintable=None): # pylint: disable=too-many-branches
        ...
        if isinstance(maintable, TableFrom):
            ...
            if maintable._subas in [None, True, False]:
                raise JoinParseError('Require alias for subquery to '
        ...
        if isinstance(val, (tuple, list)):
            for iterval in val:
                ...
                if field1.table:
                    raise JoinParseError('Expected table specified '
                                         'in JOIN ON fields.')
                if not maintable:
                    raise JoinParseError('Short format of JOIN fields are '
            ...
            for iterkey, iterval in val.items():
                ...
                if field1.table:
                    raise JoinParseError('Unexpected table on JOIN ON '
            ...
            field1 = Field(val)
            if field1.table:
                raise JoinParseError('Unexpected table specified in '
                                     'JOIN ON fields.')
    ...
    def __str__(self):
        return '{} {} ON {}'.format(
            ...
            self.table,
            ' AND '.join('{}={}'.format(key, val)
                         for key, val in self.onfields)
        )
...
class Builder(Term):
    ...
    def _from(self, *tables):
        ...
        self.table = (tableterms[0]._subas
    ...
    def _limit(self, limoff):
        ...
        if lim.pos == -1:
            ...
            if 'WHERE' not in self.terms:
                ...
                if (' AND ' in str(self.terms[whereindex + 1]) or
                        ' OR ' in str(self.terms[whereindex + 1])):
            ...
            if isinstance(values[0], dict):
                ...
                for i, value in enumerate(values):
                    ...
                    if (isinstance(value, dict) and
                            set(fields) != set(value.keys())):
                        raise InsertParseError('Inconsistent keys in '
                                               'values for INSERT.')
        if fields:
            fieldterms = [Table.parse(field, 'insert')
                          for field in always_list(fields)]
            self.terms.append('({})'.format(
                ','.join([str(field) for field in fieldterms])
            ))
        ...
        if isinstance(values[0], Builder):
            ...
            for value in values:
                if isinstance(value, Builder):
                    ...
                    raise InsertParseError('All values should be subqueries '
                                           'in INSERT.')
        else:
            insertvals = []
            for value in values:
                if isinstance(value, (tuple, list)):
                    ...
                    raise InsertParseError('Unsupported type for values '
                                           'in INSERT: {}'.format(type(value)))
            self.terms.append('VALUES')
            valterms = []
            for inval in insertvals:
                valterms.append('(%s)' % (','.join([Builder.DIALECT.value(iv)
                                                    for iv in inval])))
